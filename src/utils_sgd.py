...
def get_initialized_embeddings(approximation_rank, u_rows, z_rows):
    ...
    u_embedding = np.random.rand(u_rows, approximation_rank).astype(
            np.float128)
    z_embedding = np.random.rand(z_rows, approximation_rank).astype(
            np.float128)
    return u_embedding, z_embedding

def reconstruct(u_embedding, z_embedding, u_bias, z_bias, total_average=None):
    ...
    print('u_bias shape: ' + str(u_bias.shape))
    print('z_bias shape: ' + str(z_bias.shape))
    prod = np.dot(u_embedding, z_embedding.T)
    print('product shape: ' + str(prod.shape))
    # prod += total_average
    prod += u_bias
    prod += z_bias
    return prod
